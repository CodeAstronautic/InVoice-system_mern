...
export const registerUser = (userData, history) => async dispatch => {
  try {
    const newUser = await axios.post("http://localhost:8000/api/admin/register", userData);
...
export const loginUser = (userData) => async dispatch => {
  try {
    const user = await axios.post("http://localhost:8000/api/admin/login", userData);
    // Get the token
